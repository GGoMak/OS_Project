...
void init_proc()
{
	...
	for(int i = 0; i < RQ_NQS; i++){	// runq initialize
		list_init(&runq[i]);
	}
	...
	list_push_back(&plist, &cur_process->elem_all);	// process list 추가
	//list_push_back(&rlist, &cur_process->elem_stat);
	list_push_back(&runq[cur_process->priority/RQ_PPQ], &cur_process->elem_stat);	// runq 추가
}	
...
pid_t proc_create(proc_func func, struct proc_option *opt, void* aux)
{
	...
	*(--top) = 6; //edi
	...
	list_push_back(&runq[(opt->priority)/RQ_PPQ], &p->elem_stat);	// runq에 프로세스 추가
...
void recalculate_priority(void)
{
	...
	if(p->pid == 0){	// idle process는 priority고정
		list_pop_front(&runq[p->priority/RQ_PPQ]);	// idle process 리스트 재 삽입
		idx = p->priority/RQ_PPQ;
		list_push_back(&runq[idx], &p->elem_stat);
		return;
	}

	if(p->time_slice % 30 == 0 && p->time_slice > 0){	// 30tick마다 우선순위 재계산

		proc_getpos(p->priority, &idx);
		list_pop_front(&runq[idx]);	// list_pop
		p->priority += p->time_slice/10;

		if(p->priority > 99){	// priority 99 초과시 99로 고정
			p->priority = 99;
		}

		idx = p->priority/RQ_PPQ;
		list_push_back(&runq[idx], &p->elem_stat);	// 새로운 runq의 인덱스에 프로세스 삽입
	}
}
...
void proc_wake(void)
{
    ...
    while(!list_empty(&slist))
	{
		...
		/* You should modify this function... */
		proc_getpos(p->priority, &idx); // runq인덱스 계산

		list_remove(&p->elem_stat);	// slist에서 제거

		list_push_back(&runq[idx], &p->elem_stat);	// runq에 삽입

		p->state = PROC_RUN;	// process상태 변경
	}
}
...
void proc_sleep(unsigned ticks)
{
	...
	/* You should modify this function... */
	cur_process->time_sleep = ticks + cur_ticks;	// time_sleep 정의
...
void kernel1_proc(void* aux)
{
	...
	while(1)
	{
		...
		if(cur_process->time_used >= 200 && passed == 1){
			proc_end();
			break;
		}
	}
...
void kernel2_proc(void* aux)
{
	...
	while(1)
	{
		...
		if(cur_process->time_used >= 120 && passed == 1){
			proc_end();
			break;
		}
	}
...
void kernel3_proc(void* aux)
{
	...
	while(1)
	{
		...
		if(cur_process->time_used >= 150 && passed1 == 1 && passed2 == 1){
			proc_end();
			break;
		}
	}
